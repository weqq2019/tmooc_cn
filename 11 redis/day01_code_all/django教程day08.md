...
- 配置uWSGI
    - 添加配置文件 `项目文件夹/uwsgi.ini`
        ...
        http=127.0.0.1:8000
        # 项目当前工作目录
        chdir=/home/tarena/.../mysite7 这里需要换为项目文件夹的绝对路径
...
1，uwsgi 关闭掉   
   sudo uwsgi --stop uwsgi.pid
   ps aux|grep 'uwsgi'   如果输出只有一行，则uwsgi关闭

2, 确认 debug=True 
   启动 python3 manage.py runserver 

3，启动uwsgi 盯日志 sudo tail -f uwsgi.log


问题2：
uWSGI无法启动，则直接去看 uwsgi.log
  1，Address already use  端口重复占用， 确保你监听的端口释放出来
...
- ubuntu 下 nginx 安装
    ...
    ```shell
    备份 sudo cp -fr /etc/apt/sources.list /etc/apt/sources.list_bak
...
```shell
uwsgi启动后- 可用如下命令 监听日志；配合浏览器的请求，可快速看到对应的报错
...
uwsgi + django
#确保uwsgi关闭的前提下，检查django代码有没有问题
1，debug=True
2，python3 manage.py runserver 6000
3, 浏览器访问 127.0.0.1:6000/test_page
...
nginx:
浏览器访问 127.0.0.1/test_page
检查nginx access log 是否有请求进来
检查uwsgi.log  

如果nginx 有请求， uwsgi.log里没有日志 【页面502-uwsgi启动失败/404 - nginx转发配置】
```
